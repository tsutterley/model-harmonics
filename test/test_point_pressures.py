...
def test_point_masses(NPTS):
    ...
    G = ge * (
        1.0 + 5.2885e-3 * np.cos(th) ** 2 - 5.9e-6 * np.cos(2.0 * th) ** 2
    )
    gs = ge * (
        1.0
    ...
    grid_Ylms = gen_pressure_stokes(
        data, gs, rad_e, lon, lat, LMAX=60, LOVE=(hl, kl, ll)
    )
    point_Ylms = gen_point_pressure(
        PRESSURE, G, rad_e, LON, LAT, AREA=AREA, LMAX=60, LOVE=(hl, kl, ll)
    )
    ...
    assert np.all(
        np.abs(grid_Ylms.amplitude - point_Ylms.amplitude) < harmonic_eps
    )
