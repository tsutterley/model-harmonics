...
def harmonic_operators(
    ...
    for i, fi in enumerate(INPUT_FILES):
        ...
        if DATAFORM[i] in ('ascii', 'netCDF4', 'HDF5'):
            ...
            # netCDF4 (.nc)
            # HDF5 (.H5)
            dinput[i] = gravtk.harmonics().from_file(
                fi, format=DATAFORM[i], date=DATE
            )
        elif DATAFORM[i] in ('index-ascii', 'index-netCDF4', 'index-HDF5'):
            # read from index file
            _, dataform = DATAFORM[i].split('-')
            dinput[i] = gravtk.harmonics().from_index(
                fi, format=dataform, date=DATE
            )
    ...
    output.to_file(OUTPUT_FILE, format=DATAFORM[-1], date=DATE, **attributes)
