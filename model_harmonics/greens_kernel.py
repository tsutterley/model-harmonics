...
OUTPUTS:
    X: X-coordinates of the kernel
    Y: Y-coordinates of the kernel
    G: Green's function kernel
...
def greens_kernel(LMAX, SPACING=[], WIDTH=[], CUTOFF=0.0, LOVE=None):
    ...
    hl, kl, ll = LOVE
    # scale to unnormalize Legendre polynomials
    norm = 1.0/np.sqrt(2.0*np.arange(LMAX+1) + 1)
    # grid spacing
        ...
        for j,y in enumerate(Y):
            ...
            if (D[j,i] < CUTOFF):
                ...
                for l in range(1, LMAX+1):# LMAX+1 to include LMAX
                    ...
                    pl_lower = Ptemp[l-1]/np.sqrt(2.0*l-1.0)
                    pl_upper = Ptemp[l+1]/np.sqrt(2.0*l+3.0)
                    Pl[l] = (pl_lower - pl_upper)/2.0
            else:
                # Legendre polynomials up to LMAX+1
                alpha = np.cos(D[j,i]/rad_e)
                Ptemp,_ = gravtk.legendre_polynomials(LMAX, alpha)
                # unnormalizing Legendre polynomials
                Pl = norm*np.squeeze(Ptemp)
