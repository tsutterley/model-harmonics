...
def era5_land_read_monthly(
    ...
    if DATAFORM == 'ascii':
        ...
        twc_mean = gravtk.spatial().from_ascii(
            ddir.joinpath(mean_file), date=False, nlat=nlat, nlon=nlon
        )
    ...
    for i, y in enumerate(YEARS):
        ...
        for m, JD in enumerate(dinput['time']):
            ...
            if DATAFORM == 'ascii':
                # ascii (.txt)
                tws.to_ascii(output_file, date=True, verbose=VERBOSE)
            elif DATAFORM == 'netCDF4':
                # netCDF4 (.nc)
                tws.to_netCDF4(
