...
def gldas_scaling_factors(
    ...
    for t, gm in enumerate(grace_month):
        ...
        if DATAFORM == 'ascii':
            # ascii (.txt)
            gldas_data = gravtk.spatial().from_ascii(
                input_file,
                spacing=[dlon, dlat],
                nlat=nlat,
                nlon=nlon,
        ...
        gldas_grid.data = gravtk.harmonic_summation(
            gldas_Ylms.clm,
            gldas_Ylms.slm,
            gldas_data.lon,
            gldas_data.lat,
    ...
    attributes['kfactor']['description'] = (
        'Best fit scaling factor for GLDAS data'
    )
    ...
    attributes['error']['description'] = (
        'RMS difference between the scaled GLDAS and original GLDAS data'
    )
...
def arguments():
    ...
    parser.add_argument(
        '--end', '-e', type=int, default=228, help='Ending GRACE/GRACE-FO month'
    )
