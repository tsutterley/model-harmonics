#!/usr/bin/env python
"""
gldas_read_3hourly.py
Written by Tyler Sutterley (05/2026)

Reads GLDAS 3-hour datafiles from http://ldas.gsfc.nasa.gov/gldas/
Adding Soil Moisture, snow water equivalent (SWE) and total canopy storage
Calculates the 3-hourly anomaly in total water content on a daily basis
...
UPDATE HISTORY:
    Updated 05/2026: use itertools group by to read files by date
...
def gldas_read_3hourly(
    ...
    # subdirectory for model 3-hourly products at spacing for version
    ...
    regex_pattern = r'GLDAS_{0}{1}_(3H|3H_EP)'.format(MODEL, SPATIAL)
    regex_pattern += r'\.A(\d{8})\.(\d{4})\.(\d{3})'
    ...
    for i, yr in enumerate(year_dir):
        ...
        for k, g in itertools.groupby(f, lambda x: rx.search(x.name).group(2)):
            # file output file
            YY, MM, DD = k[:4], k[4:6], k[6:8]
            args = (MODEL, SPATIAL, k, suffix[DATAFORM])
            FILE = 'GLDAS_{0}{1}_TWC_{2}.{3}'.format(*args)
            output_file = ddir.joinpath(FILE)
            TEST = False
            # Checking if output file exists
            if output_file.exists():
                # check last modification time of local file
                file1_mtimes = [fi.stat().st_mtime for fi in g]
                file2_mtime = output_file.stat().st_mtime
                # if any input file is newer: overwrite the output file
                if (file1_mtimes > file2_mtime).any():
                    TEST = True
            else:
                TEST = True
            # if not overwriting and file exists: skip to next day
            ...
            s = gravtk.spatial()
            s.data = np.zeros((nlat, nlon, 8))
            s.mask = np.ones((nlat, nlon, 8), dtype=bool)
            s.time = np.zeros((8))
            # for each GRIB/netCDF4 file
            for j, d in enumerate(g):
                # Getting date information from file
                EP, YYMMDD, HH, VF, SFX = rx.findall(d.name).pop()
                # read GRIB or netCDF4 file
                if SFX == 'grb':
                    SM, SWE, CW, s.lat, s.lon, s.fill_value = grib_twc_read(d)
                elif SFX == 'nc4':
                    SM, SWE, CW, s.lat, s.lon, s.fill_value = ncdf_twc_read(d)
                elif SFX == 'grb.SUB.nc4':
                    SM, SWE, CW, s.lat, s.lon, s.fill_value = subset_twc_read(d)
                # calculate total water content (TWC)
                ii, jj = np.nonzero((SWE != s.fill_value) & (~twc_mean.mask))
                twc = SM[ii, jj] + SWE[ii, jj] + CW[ii, jj]
                # converting from kg/m^2 to cm water equivalent (cmwe)
                s.data[ii, jj, j] = 0.1 * twc
                # calculating the TWC anomaly by removing the mean field
                s.data[ii, jj, j] -= twc_mean.data[ii, jj]
                # set the mask for valid points
                s.mask[ii, jj, j] = False
                # calculate date
                (s.time[j],) = gravtk.time.convert_calendar_decimal(
                    np.int64(YY), np.int64(MM), np.int64(DD), hour=np.int64(HH)
                )

            # output to file
            if DATAFORM == 'ascii':
                # ascii (.txt)
                s.to_ascii(output_file, date=True, verbose=VERBOSE)
            elif DATAFORM == 'netCDF4':
                # netCDF4 (.nc)
                s.to_netCDF4(
                ...
                # HDF5 (.H5)
                s.to_HDF5(output_file, date=True, verbose=VERBOSE, **attributes)
...
def arguments():
    parser = argparse.ArgumentParser(
        description="""Reads GLDAS 3-hourly datafiles to calculate
