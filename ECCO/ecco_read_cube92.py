...
Calculates monthly ocean bottom pressure anomalies from the ECCO2 Cube92 ocean model
...
#-- PURPOSE: read ECCO2 Cube92 ocean bottom pressure data and calculate monthly
#-- anomalies in absolute ocean bottom pressure
def ecco_read_cube92(ddir, YEARS, RANGE=None, DATAFORM=None, VERBOSE=False,
    MODE=0o775):
    #-- input and output subdirectories
    sd1 = 'cube92_latlon_quart_90S90N'
    sd2 = 'ECCO_Cube92_AveRmvd_OBP'
    ...
    if (DATAFORM == 'ascii'):
        #-- ascii (.txt)
        obp_mean = gravity_toolkit.spatial(spacing=[dlon,dlat],
    ...
    output_average_file = 'ECCO_Cube92_Global_Average_OBP.txt'
    ...
    for t,fi in enumerate(flist):
        ...
        obp_anomaly = gravity_toolkit.spatial(spacing=[dlon,dlat],nlon=nlon,
            nlat=nlat,fill_value=PHIBOT.fill_value)
...
def output_data(data,FILENAME=None,DATAFORM=None,VERBOSE=False):
    TITLE = 'Ocean_Bottom_Pressure_from_ECCO2_Cube92_Model'
    if (DATAFORM == 'ascii'):
        #-- ascii (.txt)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="""Reads monthly ECCO2 Cube92 ocean bottom
            pressure data and calculates monthly anomalies
            """
    )
    ...
    parser.add_argument('--year','-Y',
        type=int, nargs='+', default=range(2000,2018),
    ...
    #-- run program
    ecco_read_cube92(args.directory, args.year, RANGE=args.mean,
