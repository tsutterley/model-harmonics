...
def yearly_file_mean(
    input_dir, REGION, VERSION, PRODUCT, START, END, GZIP=False
):
    ...
    for t in range(n_files):
        ...
        for m in range(12):
            ...
            tdec[c] = gravtk.time.convert_calendar_decimal(
...
def compressed_file_mean(
    ...
    # calculate time in year-decimal
    tdec = gravtk.time.convert_calendar_decimal(
        YY, MM, day=DD, hour=hh, minute=mm, second=ss
    )
