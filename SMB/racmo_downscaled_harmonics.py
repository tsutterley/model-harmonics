...
def racmo_downscaled_harmonics(
    model_file,
    VARIABLE,
    MASKS=None,
    LMAX=None,
    ...
    ps_scale = mdlhmc.spatial.scale_factors(
        gridlat[indy, indx], flat=flat, reference_latitude=standard_parallel
    )
...
def main():
    ...
    try:
        info(args)
        output_file = racmo_downscaled_harmonics(
            args.infile,
            args.product,
            MASKS=args.mask,
            LMAX=args.lmax,
