...
def merra_smb_mean(DIRECTORY, PRODUCT, RANGE=None, DATAFORM=None,
    ...
    if (DATAFORM == 'ascii'):
        ...
        #-- netcdf (.nc)
        merra_mean.to_netCDF4(os.path.join(DIRECTORY,FILE),varname=PRODUCT,
            UNITS='mm w.e.', LONGNAME='Equivalent_Water_Thickness',
            TITLE=merra_products[PRODUCT], verbose=VERBOSE)
    elif (DATAFORM == 'HDF5'):
        #-- HDF5 (.H5)
        merra_mean.to_HDF5(os.path.join(DIRECTORY,FILE),varname=PRODUCT,
            UNITS='mm w.e.', LONGNAME='Equivalent_Water_Thickness',
            TITLE=merra_products[PRODUCT], verbose=VERBOSE)
