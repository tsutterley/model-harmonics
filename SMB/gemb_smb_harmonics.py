...
def gemb_smb_harmonics(
    ...
    ps_scale = mdlhmc.spatial.scale_factors(
        gridlat[indy, indx], flat=flat, reference_latitude=reference_latitude
    )
    ...
    for t in range(nt - 1):
        ...
        YLMS = gravtk.gen_point_load(
            scaled, lon, lat, LMAX=LMAX, MMAX=MMAX, UNITS=UNITS, LOVE=LOVE
        )
...
def arguments():
    ...
    parser.add_argument(
        'infile', type=pathlib.Path, help='GEMB SMB file to run'
    )
