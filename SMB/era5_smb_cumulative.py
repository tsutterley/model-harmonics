...
def era5_smb_cumulative(
    ...
    if DATAFORM == 'ascii':
        ...
        era5_mean = (
            gravtk.spatial()
            .from_netCDF4(
                DIRECTORY.joinpath(mean_file), date=False, varname='SMB'
            )
            .squeeze()
        )
    elif DATAFORM == 'HDF5':
        ...
        era5_mean = (
            gravtk.spatial()
            .from_HDF5(DIRECTORY.joinpath(mean_file), date=False, varname='SMB')
            .squeeze()
        )
    ...
    for i, era5_flux_file in enumerate(input_files):
        ...
        output = gravtk.spatial(nlat=nlat, nlon=nlon, fill_value=fill_value)
