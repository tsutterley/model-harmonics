...
def racmo_smb_harmonics(
    ...
    fd['time'] = gravtk.time.convert_calendar_decimal(
    ...
    dlon = np.radians(np.abs(rlon[1] - rlon[0]))
    dlat = np.radians(np.abs(rlat[1] - rlat[0]))
    _, gridrlat = np.meshgrid(rlon, rlat)
    fd['area'] = np.radians(np.cos(gridrlat)) * (a_axis**2) * dlon * dlat
    ...
    fd['lat'] = mdlhmc.spatial.geocentric_latitude(
        fd['lon'], gridlat, a_axis=a_axis, flat=flat
    )
    ...
    for t in range(nt - 1):
        ...
        racmo_Ylms = gravtk.gen_point_load(
            ptms, lon, lat, LMAX=LMAX, MMAX=MMAX, UNITS=1, LOVE=LOVE
        )
