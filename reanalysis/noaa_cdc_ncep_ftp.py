...
def noaa_cdc_ncep_ftp(
    ...
    remote_files, remote_mtimes = gravtk.utilities.ftp_list(
        HOST, timeout=TIMEOUT, basename=True, pattern=R1, sort=True
    )
    for fi, mtime in zip(remote_files, remote_mtimes):
        ...
        gravtk.utilities.from_ftp(
            posixpath.join(*HOST, fi),
            local=local_file,
            timeout=TIMEOUT,
            hash=MD5,
            verbose=True,
    ...
    if INVARIANT:
        ...
        MD5 = gravtk.utilities.get_hash(local_file)
        gravtk.utilities.from_ftp(
            posixpath.join(*HOST, 'hgt.sfc.nc'),
            local=local_file,
            timeout=TIMEOUT,
            hash=MD5,
            verbose=True,
