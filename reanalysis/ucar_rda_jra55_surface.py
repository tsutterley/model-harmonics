...
def ucar_rda_download(
    ...
    rx3 = re.compile(
        regex_pattern.format(prefix, r'\d{4}', r'\.tar').encode('utf-8')
    )
    ...
    for YY in YEARS:
        ...
        if remote_tar_file:
            remote_lines = [
                i for i, f in enumerate(valid_lines) if rx.search(f.name)
            ]
        else:
            remote_lines = [
                i for i, f in enumerate(valid_lines) if rx.search(f)
            ]
