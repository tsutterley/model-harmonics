...
def ecco_llc_tile_harmonics(
    ...
    input_invariant_file = ddir.joinpath(
        f'ECCO-{MODEL}', 'nctiles_monthly', 'ECCO-GRID.nc'
    )
    ...
    input_geoid_file = ddir.joinpath(
        f'ECCO-{MODEL}', 'nctiles_monthly', 'ECCO-EGM2008.nc'
    )
    ...
    for f in sorted(FILES):
        ...
        for k in range(Nt):
            ...
            Ylms = mdlhmc.gen_point_pressure(
                obp_tile,
                gamma_tile,
                rad_tile,
                lon,
        ...
        obp_Ylms.to_file(output_file, format=DATAFORM, date=True)
